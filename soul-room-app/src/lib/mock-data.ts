...
// Special founder VIP entry (level 999)
const FOUNDER_VIP_INFO = {
  level: 999, name: 'Founder', badge: '👑',
  xpRequired: 0, monthlyMaintenanceXp: 0, levelUpVpBonus: 0,
  giftEarningRate: 50,
  hasCustomBorderColor: true, hasExtendedSparkCall: true, hasHideLastSeen: true,
  hasGhostMode: true, hasEntranceAnimation: true, hasReadReceiptControl: true,
  hasPlatinumRooms: true, hasInvisibleBrowsing: true, hasDiamondEntrance: true,
  hasWorldCreation: true, reducedSafeExitTollVp: 0, hasConcierge: true,
  hasPriorityMatchmaking: true, hasLegendaryFrame: true, hasCoHosting: true,
  hasIncognitoSpark: true, hasUnlimitedWorlds: true, hasCustomAnimation: true,
  hasGlobalLeaderboard: true,
  tagline: 'Founding member — all privileges forever.',
  keyPrivilege: 'Infinite VP, all features unlocked, no restrictions',
};

// Helper: get VIP level info for a user
export function getVipInfo(level: number) {
  if (level >= 9) return FOUNDER_VIP_INFO;
