...
import { getVipInfo, INTEREST_TAGS, MOCK_USERS } from '@/lib/mock-data';
...
export default function SparkPage() {
  ...
  useEffect(() => {
    if (!user) return;
    if (isDemoMode) {
      // Use mock data in demo — no Supabase call needed
      const demoCandidates: Candidate[] = MOCK_USERS.map((u) => ({
        id: u.id,
  ...
  const handleVote = useCallback(async (sparked: boolean) => {
    if (!user || !currentCandidate) return;
    if (isDemoMode) {
      // Simulate a ~50% match rate in demo
      const mutual = sparked && Math.random() > 0.5;
      setLastMatchResult({ mutual });
      setPhase('result');
      return;
    }
    ...
    setPhase('result');
  }, [user, currentCandidate, isDemoMode]);
