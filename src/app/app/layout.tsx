...
function NavIcon({ type, active }: { type: string; active: boolean }) {
  const color = active ? '#FF4B6E' : '#A87DC9';
  switch (type) {
    case 'home':
      return (
...
function ThemeToggle() {
  ...
  const nextTheme = theme === 'dark' ? 'light' : theme === 'light' ? 'solarized' : 'dark';
    ...
    <div className="flex flex-col min-h-screen max-w-lg mx-auto bg-dark-900 relative">
      ...
      <header className={`sticky top-0 z-50 glass px-4 py-3 flex items-center justify-between border-b border-white/5 transition-transform duration-300 ${showNav ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/app/vp" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-dark-700/60 hover:bg-dark-600 transition-colors border border-white/5">
            <span className="text-sm">💎</span>
            <span className="text-sm font-semibold text-white">{formatNumber(vpBalance)}</span>
          </Link>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[10px] mt-1 font-medium transition-colors ${isActive ? 'text-[#FF4B6E]' : 'text-[#A87DC9]'}`}>
                  {item.label}
                </span>
                {item.badge > 0 && (
